...
def _normalize_capability_mode(raw: Any) -> str:
...
def _capability_policy_text(mode: str) -> str:
    m = _normalize_capability_mode(mode)
    if m == "copy_only":
...
def _capability_focus_for_turn(mode: str, turn_idx: int) -> str:
    m = _normalize_capability_mode(mode)
    if m == "copy_only":
        return "copywriting"
    if m == "image_only":
...
def build_user_simulator_config(cfg: AuthConfig, llm_eval_cfg: LLMEvalConfig) -> UserSimulatorConfig:
    ...
    return UserSimulatorConfig(
        ...
        capability_mode=_normalize_capability_mode(
...
def _render_workspace_snapshot_for_user_sim(snapshot: dict[str, Any] | None) -> str:
    ...
    with raw_events_path.open("r", encoding="utf-8", errors="ignore") as fp:
        for line in fp:
            ...
            if event_type not in {"TOOL_CALL_DELTA", "TEXT_DELTA"}:
                all_paths.update(_extract_workspace_paths(json.dumps(event_raw, ensure_ascii=False)))
...
def _render_user_sim_scenario(template: str, max_turns: int, capability_mode: str) -> str:
    mode = _normalize_capability_mode(capability_mode)
    return (
        (template or "")
        .replace("{{MAX_TURNS}}", str(max_turns))
        .replace("{{CAPABILITY_MODE}}", mode)
        .replace("{{CAPABILITY_POLICY}}", _capability_policy_text(mode))
    )
...
def _build_role_profile(role_json: dict[str, Any]) -> str:
    labels = [
        ...
        ("speaking_habits", "表达习惯"),
    ]
    lines: list[str] = []
    for key, label in labels:
        value = role_json.get(key)
...
def _parse_generated_role(content: str) -> GeneratedRole:
    ...
    if isinstance(parsed, dict):
        ...
        if not role_profile:
            role_profile = content.strip()
        return GeneratedRole(role_name=role_name, role_profile=role_profile, role_json=parsed)

    fallback_name = f"随机客户-{uuid.uuid4().hex[:6]}"
    fallback_profile = content.strip() or "随机客户，偏好简洁直接沟通，注重实用性与时效。"
    return GeneratedRole(
        role_name=fallback_name,
        role_profile=f"- 角色名: {fallback_name}\n- 描述: {fallback_profile}",
        role_json={"role_name": fallback_name, "description": fallback_profile},
    )


def generate_role_for_session(sim_cfg: UserSimulatorConfig) -> GeneratedRole:
    role_user_prompt = (
        (sim_cfg.role_user_prompt or "")
        .replace("{{MAX_TURNS}}", str(sim_cfg.max_turns))
        .replace("{{REQUIRED_NOTEBOOK_CLEAR_TEXT}}", REQUIRED_NOTEBOOK_CLEAR_TEXT)
    )
    content, _ = _call_user_sim_model(
        sim_cfg=sim_cfg,
        system_prompt=sim_cfg.role_system_prompt,
        user_prompt=role_user_prompt,
...
def generate_user_turn_with_simulator(
    ...
    scenario_prompt = _render_user_sim_scenario(
    ...
    workspace_text = _render_workspace_snapshot_for_user_sim(workspace_snapshot)
    current_focus = _capability_focus_for_turn(sim_cfg.capability_mode, turn_idx)
    ...
    raise RuntimeError(f"user_simulator 输出为空: {latest_content[:200]}")
...
def execute_turn(
    ...
    while True:
        ...
        for raw in resp.iter_lines(decode_unicode=True):
            ...
            if event_type not in {"TOOL_CALL_DELTA", "TEXT_DELTA"}:
                event_workspace_paths.update(_extract_workspace_paths(json.dumps(event, ensure_ascii=False)))
        ...
        if pending_tool_call is not None:
            ...
            event_workspace_paths.update(_extract_workspace_paths(json.dumps(tool_message, ensure_ascii=False)))
            continue
